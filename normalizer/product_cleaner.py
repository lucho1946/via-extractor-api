...
# ==========================================================
# EXTRAER PESO DESDE DETALLES TÉCNICOS
# ==========================================================
def extract_weight(technical_details: dict, bullets: list, title: str) -> Optional[str]:

    weight_keywords = [
        "peso", "weight", "item weight", "product weight",
        "net weight", "gross weight", "peso neto", "peso bruto"
    ]
    ...
    text_blob = " ".join(bullets) + " " + (title or "")
    match = re.search(r"\b(\d+(?:\.\d+)?\s?(?:kg|lb|lbs|g|oz|kilogram|pound)s?)\b", text_blob, re.IGNORECASE)
    if match:
...
def filter_irrelevant_specs(technical_details):
    ...
    ignore_keywords = [
        "asin", "fabricante", "manufacturer", "pais", "country",
        "date", "available", "first", "origen", "ranking", "seller",
        "brand", "customer", "review", "opinión", "rating", "amazon",
        "best sellers", "clasificación", "opinión media",
        "usuarios sugeridos", "estilo", "color", "tamaño", "size", "upc"
    ]
...
def clean_raw_product(raw_data: dict):
    ...
    referencia = detect_reference(
        title_raw, technical_details, product_information,
        bullets, product_description, url_origen
    )
    ...
    # EXTRAER PESO
    peso = extract_weight(technical_details, bullets, title_raw)
